...
namespace Tara
{
	...
	{
		public UnityEvent OnTargetsInRangeChanged;
		...
		private void GenerateCollidersForRanges()
		{
			...
			for (int i = 0; i < 3; i++)
			{
				...
				rangeColliders[i].TargetRangeFilterer = this;
			}
		}
		...
		private void TargetEnteredRange(RangeType rangeType, Target target)
		{
			switch (rangeType)
			{
				...
				case RangeType.TargetingRange:
					...
					OnTargetsInRangeChanged?.Invoke();
					break;
			}
		}
		...
		public bool IsInStoppingRange(Target target) => targetsInStoppingRange.Contains(target);
		public bool IsInShootingRange(Target target) => targetsInShootingRange.Contains(target);
		public bool IsInTargetingRange(Target target) => targetsInTargetingRange.Contains(target);

		public bool IsHighPriority(Target target) => targetList.IsHighPriority(target);

		private int GetHighestPriorityLevelInList(List<Target> targets)
		{
		...
		{
			Target target = null;
			List<Target> validTargets = targetList.GetTargetsWithPriorityLevel(GetHighestPriorityLevelInList(targetsInTargetingRange));
